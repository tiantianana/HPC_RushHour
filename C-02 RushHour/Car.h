...
public:
	...
	size_t hash() const{
		return size_t(x ^ (y << 1)); // Only those two things need be checked, the rest never changes (and is thus a bad criterion to hash by)

	};

	bool operator == (const Car& other) const{
		if(x == other.x && y == other.y) {return true;} //Only those two things need be checked, the rest never changes!
		return false;
	};
